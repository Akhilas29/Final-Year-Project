...
# Set the page config
st.set_page_config(page_title='Predicting Glycemic Index Contorl with Diabetic Prediction', layout='wide')
...
# Title
st.title('Predicting Glycemic Index Contorl with Diabetic Prediction')


# Sidebar for user input
st.sidebar.header('Glycemic Index Contorl')
#st.sidebar.write(hyperchloremia_text)

# Sidebar content
st.sidebar.write('''<small>Predicting future glycemic events such as hypoglycemia, hyperglycemia, and normal for type 1 diabetes (T1D) remains a significant and challenging issue.</small>''', unsafe_allow_html=True)
...
with tab2:
        ...
        with col2:
            ...
            if input_data is not None:
                ...
                # Display prediction
                if prediction[0] == 1:
                    st.error("The patient is likely to have Glycemic considering the factors.")
                else:
                    st.success("The patient is unlikely to have Glycemic considering the factors.")
